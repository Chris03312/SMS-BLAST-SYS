...
export function initDb() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS remote_stats_snapshots (
      id                   TEXT PRIMARY KEY,
    ...
    CREATE INDEX IF NOT EXISTS idx_remote_stats_install_id ON remote_stats_snapshots(install_id);
    CREATE INDEX IF NOT EXISTS idx_remote_stats_timestamp ON remote_stats_snapshots(timestamp);
  `);
  ...
  if (envPass) {
    ...
    if (row) {
      db.prepare('UPDATE users SET password_hash = ?, role = ?, active = 1 WHERE id = ?')
        .run(hash, 'admin', row.id);
    } else {
      db.prepare('INSERT INTO users (id, username, password_hash, display_name, role) VALUES (?, ?, ?, ?, ?)')
        .run(uuidv4(), adminUser, hash, 'Admin', 'admin');
    }
    console.log(`[db] Admin "${adminUser}" ensured from ADMIN_PASSWORD env.`);
    return;
  }
