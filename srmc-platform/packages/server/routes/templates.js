...
router.get('/', (req, res) => {
  try {
    ...
    if (isAdmin) {
      sql = `
        SELECT t.*, u.display_name as creator_name, c.name as campaign_name
        FROM templates t
...
router.post('/', (req, res) => {
  try {
    ...
    const id = uuidv4();
    db.prepare('INSERT INTO templates (id, name, body, category, variables, created_by, boss_numbers, campaign_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
      .run(id, name, body, category || 'transactional', JSON.stringify(variables || []), req.user.id, boss_numbers || '', campaign_id || null);
