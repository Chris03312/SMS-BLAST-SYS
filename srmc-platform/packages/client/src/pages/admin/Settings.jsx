          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0, paddingBottom: 72 }}>
            ...
            <SectionBlock sectionKey="account" label="Account & Branding" desc="Your workspace identity shown in agent consoles and exports." refs={sectionRefs}>
              <FieldRow cols={2}>
                ...
                <Field label="Default sender ID" help="Up to 6 alphanumeric characters">
            ...
            <SectionBlock sectionKey="preferences" label="Sending Behaviour" desc="Default delay, allowed sending window, and per-agent daily cap." refs={sectionRefs}>
              ...
              <FieldRow cols={2}>
                <Field label="Max broadcast duration" help="Auto-cancel broadcasts running longer than this. 0 = unlimited.">
                  <div style={{ display: 'flex' }}>
                    <input className="input mono" type="number" min="0" value={form.max_broadcast_duration_minutes || 0} onChange={e => set('max_broadcast_duration_minutes', e.target.value)}
                      style={{ fontSize: 12, borderTopRightRadius: 0, borderBottomRightRadius: 0, borderRight: 'none', maxWidth: 160 }} />
                    <span style={{ padding: '10px 10px', background: 'var(--bg-soft)', border: '1px solid var(--line)', borderRadius: '0 8px 8px 0', fontSize: 11, color: 'var(--ink-3)', whiteSpace: 'nowrap' }}>
                      minutes
                    </span>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 6 }}>
                    Broadcasts are auto-cancelled if they run longer than this limit.
                  </div>
                </Field>
                <Field label="Broadcasting hours" help="Active broadcasts pause outside these hours.">
                  <div style={{ display: 'flex', gap: 6 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 10, color: 'var(--ink-4)', marginBottom: 2 }}>Start</div>
                      <input className="input mono" type="time" value={form.window_start || '00:00'} onChange={e => set('window_start', e.target.value)} style={{ fontSize: 12, width: '100%' }} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 10, color: 'var(--ink-4)', marginBottom: 2 }}>Stop</div>
                      <input className="input mono" type="time" value={form.window_end || '23:59'} onChange={e => set('window_end', e.target.value)} style={{ fontSize: 12, width: '100%' }} />
                    </div>
                  </div>
                  <div style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 6 }}>
                    Broadcasts that are active outside this window will pause automatically. Uses the configured timezone.
                  </div>
                </Field>
              </FieldRow>

              {/* ── Global Broadcast Kill Switch ── */}
              <Field label="Global broadcast pause" help="Pause ALL broadcasts instantly. No new broadcasts can be created while paused.">
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 4 }}>
                  <button
                    type="button"
                    onClick={async () => {
                      const isPaused = form.broadcasts_globally_paused === 'true';
                      try {
                    ...
                    style={{
                      ...
                      background: form.broadcasts_globally_paused === 'true' ? 'var(--err, #EF4444)' : 'var(--ok, #059669)',
                      color: '#fff',
                      transition: 'background 0.2s',
                    }}
                  >
                    {form.broadcasts_globally_paused === 'true' ? '⏸  Paused — Tap to Resume' : '▶  Active — Tap to Pause'}
                  </button>
                  <span style={{ fontSize: 12, color: 'var(--ink-3)' }}>
                    {form.broadcasts_globally_paused === 'true'
                      ? 'All broadcasting is currently paused.'
                      : 'Broadcasting is active.'}
                  </span>
                </div>
                <div style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 6 }}>
                  When paused: running broadcasts pause, no new broadcasts can be created. Toggle back to resume all.
                </div>
              </Field>

              <Field label="Timezone" style={{ marginBottom: 18 }}>
                <select className="input" value={form.timezone || 'Asia/Manila'} onChange={e => set('timezone', e.target.value)}
                  ...
                  <option value="Asia/Singapore">Asia/Singapore (UTC+8) — Singapore</option>
                  <option value="America/New_York">America/New_York (UTC-5) — US Eastern</option>
                  <option value="America/Chicago">America/Chicago (UTC-6) — US Central</option>
                  <option value="America/Los_Angeles">America/Los_Angeles (UTC-8) — US Pacific</option>
                  <option value="Europe/London">Europe/London (UTC+0) — UK</option>
                  <option value="Australia/Sydney">Australia/Sydney (UTC+11) — Sydney</option>
                  <option value="Pacific/Auckland">Pacific/Auckland (UTC+13) — New Zealand</option>
                  <option value="UTC">UTC</option>
                </select>
                <div style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 6 }}>
                  Controls how sending windows and all date/time displays are shown.
                </div>
              </Field>
            </SectionBlock>
            ...
            <SectionBlock sectionKey="sender-ids" label="Sender IDs & Opt-out" desc="Registered sender names and how STOP messages are handled." refs={sectionRefs}>
              <FieldRow cols={2}>
                <Field label="Default sender ID" help="Alphanumeric sender ID (max 6 chars)">
                  <input className="input mono" value={form.sender_id || ''} onChange={e => set('sender_id', e.target.value)} style={{ fontSize: 12 }} />
                </Field>
...
function DangerAction({ label, desc, btnLabel, onAction }) {
  return (
    <div style={{
      display: 'flex', justifyContent: 'space-between', alignItems: 'center',
      padding: '14px 0',
      borderBottom: '1px solid var(--err-line)',
    }}>
      <div>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink-1)' }}>{label}</div>
        <div style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 2 }}>{desc}</div>
      </div>
      <button type="button" className="btn-danger" onClick={onAction} style={{ flexShrink: 0, marginLeft: 20 }}>
        {btnLabel}
      </button>
    </div>
  );
